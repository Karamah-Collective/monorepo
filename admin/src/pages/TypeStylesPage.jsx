...
function TypeStyleRow({ item, saving, onSave }) {
  ...
  const handleUpload = (file) => {
    if (!file) return;
    if (!/^image\/(svg\+xml|png|webp|jpeg|gif|avif|bmp|x-icon|vnd\.microsoft\.icon)$/.test(file.type)) {
  ...
  return (
    <article className={`pp-type-style-card${open ? " open" : ""}`}>
      <button className="pp-type-style-summary" type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open}>
        <TypeIconPreview iconKeyOrDataUrl={icon} color={color} size={19} />
        <div className="pp-type-style-title">
          <h2>{item.label}</h2>
          <p>{isCustomIcon(icon) ? "Custom icon" : BUILT_IN_TYPE_ICONS.find((option) => option.key === icon)?.label || "Built-in icon"}</p>
        </div>
        <span className="pp-type-style-color-dot" style={{ backgroundColor: color }} />
      ...
      {open && (
        <div className="pp-type-style-editor">
          ...
          <div className="pp-type-icon-grid" aria-label={`Built-in icons for ${item.label}`}>
            {BUILT_IN_TYPE_ICONS.map((iconOption) => (
              ...
              >
                <TypeIconPreview iconKeyOrDataUrl={iconOption.key} color={color} size={15} />
          ...
          <label className="pp-type-upload">
            <span>{isCustomIcon(icon) ? "Replace custom icon" : "Upload custom icon"}</span>
            <small>SVG, PNG, WebP, JPEG, GIF, AVIF, BMP, ICO under 48 KB</small>
            <input type="file" accept="image/svg+xml,image/png,image/webp,image/jpeg,image/gif,image/avif,image/bmp,image/x-icon,image/vnd.microsoft.icon,.ico" disabled={saving} onChange={(event) => handleUpload(event.target.files?.[0])} />
          </label>
        </div>
      )}
    </article>
...
export default function TypeStylesPage() {
  ...
  const categoryOrder = ["space_space_type", "restaurant_restaurant_type", "service_service_type"].filter((category) => grouped[category]?.length);
  const activeItems = grouped[activeCategory] || grouped[categoryOrder[0]] || [];

  const saveTypeStyle = (item) => {
    updateTypeStyle.mutate(
      { category: item.category, tagId: item.tagId, icon: item.icon, color: item.color },
      {
  ...
  return (
    <div className="pp-page">
      ...
      <p className="pp-page-lead">Set the marker language for each type. Built-ins cover the common cases; uploads are for special icons the library does not cover.</p>
      {isLoading && <p>Loading...</p>}
      {error && <p className="pp-error-text">{error.message}</p>}
      {!!categoryOrder.length && (
        <>
          <div className="pp-type-style-tabs" role="tablist" aria-label="Type categories">
          ...
          <section className="pp-type-style-section">
            <div className="pp-type-style-list">
              {activeItems.map((item) => (
                <TypeStyleRow key={`${item.category}:${item.tagId}`} item={item} saving={updateTypeStyle.isPending} onSave={saveTypeStyle} />
              ))}
            </div>
          </section>
        </>
