...
import { usePendingEdits, useApproveEdit, useRejectEdit } from "../api/queries.js";
...
function CurrentVsProposed({ current, proposed, field }) {
  ...
  if (!proposedVal || proposedVal === currentVal) return <span>{currentVal || "—"}</span>;
  return (
...
export default function PendingEditsPage() {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.display({
        id: "name",
        header: "Name",
        cell: (info) => <CurrentVsProposed current={info.row.original.current} proposed={info.row.original.name} field="name" />,
      }),
      columnHelper.display({
        id: "address",
        header: "Address",
        cell: (info) => <CurrentVsProposed current={info.row.original.current} proposed={info.row.original.address} field="address" />,
      }),
  ...
  return (
    <div>
      ...
      {data && <DataTable data={data} columns={columns} getRowId={(row) => row.rowId} emptyMessage="No pending edits." />}
    </div>
  );
}
