      ...
      columnHelper.accessor("boycott", {
        ...
        cell: (info) => (
          <ToggleCell
            ...
            ariaLabel={`Boycott ${info.row.original.name}`}
            tone="danger"
      ...
      columnHelper.accessor((row) => (row.disabled ? "Disabled" : "Visible"), {
        ...
        cell: (info) => (
          <ToggleCell
            ...
            ariaLabel={`Show ${info.row.original.name} on map`}
              ...
              updateDisabled.mutate(
                { placeId: info.row.original.id, disabled: !visible },
                {
                  onSuccess: () =>
                    showToast(
                      visible
                        ? "Place enabled on map"
                        : "Place disabled on map",
                    ),
                  onError: (e) => showToast(e.message, "error"),
                },
              )
            ...
            <div className="pp-action-cell">
              ...
              <button
                ...
                onClick={() => {
                  if (
                    !window.confirm(
                      `Remove "${place.name}" from the database? This also removes its reviews, saved references, events, app links, and social videos.`,
                    )
                  )
                    return;
                  deletePlace.mutate(
