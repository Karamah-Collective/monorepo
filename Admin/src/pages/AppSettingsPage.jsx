...
import {
  ...
  validateAppSettings,
} from "../../../Maps/src/app-settings-schema.js";
import { MAP_PRESETS } from "../../../Maps/src/app-controls-schema.js";
...
const GROUPS = [
  [
    "Appearance",
    "Set the public app's default visual tone for visitors who have not saved their own preferences.",
  ],
  [
    "Map behavior",
    "Choose the default map detail, marker feel, zoom limits, and optional overlays.",
  ],
  [
    "Search & discovery",
    "Tune list cards, search behavior, and the empty state visitors see when no places match.",
  ],
  [
    "Navigation",
    "Control shortcuts and the order of sections in the Menu sheet.",
  ],
  [
    "Categories",
    "Rename the public category labels without touching taxonomy code.",
  ],
  ["Startup", "Control loading, onboarding, and the fallback map position."],
  [
    "Submissions",
    "Pause or tighten community contribution workflows from one place.",
  ],
...
const FIELD_GROUPS = {
  Startup: [
    ...
    "defaultZoom",
    "promosEnabled",
    "eventsShortcutEnabled",
    "sponsorCarouselEnabled",
  ],
  Content: [
    "noticeEnabled",
    "noticeText",
...
function labelFor(name) {
  return (
    APP_SETTINGS_FIELDS[name].label ||
    name.replace(/([A-Z])/g, " $1").replace(/^./, (ch) => ch.toUpperCase())
  );
}
...
function SettingField({ name, draft, update }) {
  ...
  const hint =
    field.hint ||
    (!field.options && field.type === "string" && field.maxLength
      ? `${String(draft[name] || "").length} / ${field.maxLength}`
      : "");
    ...
    <div className="pp-field">
      ...
      {field.options ? (
        ...
        <input
          ...
          max={field.max}
          step={
            field.type === "number" && Number.isInteger(field.default)
              ? "1"
              : "any"
          }
      ...
      {hint && (
        <span className="pp-field-hint" id={`hint-${name}`}>
          {hint}
        </span>
      )}
    </div>
...
function SettingsGroup({ group, description, draft, update }) {
  const keys = fieldKeysForGroup(group).filter((key) =>
    Object.hasOwn(APP_SETTINGS_FIELDS, key),
  );
  if (!keys.length) return null;
  const booleans = keys.filter(
    (key) => APP_SETTINGS_FIELDS[key].type === "boolean",
  );
  ...
  return (
    ...
    >
      <div className="pp-settings-section-head">
        <h2 id={`settings-${group.replaceAll(" ", "-").toLowerCase()}`}>
          {group}
        </h2>
      ...
      {group === "Map behavior" && (
        <div className="pp-settings-presets" aria-label="Map presets">
          {Object.entries(MAP_PRESETS).map(([name, values]) => (
            <button
              key={name}
              className="pp-btn pp-btn-inline"
              type="button"
              onClick={() => {
      ...
      {booleans.map((name) => (
        <ToggleRow key={name} name={name} draft={draft} update={update} />
      ))}
      {fields.length > 0 && (
      ...
      {group === "Content" && draft.noticeText && (
        <aside
          className="pp-settings-preview"
          aria-label="Community notice preview"
        >
          <small>
            Preview{!draft.noticeEnabled ? " · hidden in the app" : ""}
          </small>
...
function SettingsEditor({ data, reload, reloading }) {
  ...
  const changedKeys = useMemo(
    () =>
      Object.keys(DEFAULT_APP_SETTINGS).filter(
        (key) => draft[key] !== saved.settings[key],
      ),
  ...
  const save = (event) => {
    ...
    mutation.mutate(
      { settings: draft, revision: saved.revision },
      {
        onSuccess: (result) => {
          setSaved(result);
  ...
  return (
    <form className="pp-settings-form" onSubmit={save}>
      ...
      <fieldset className="pp-settings-fields" disabled={mutation.isPending}>
        {GROUPS.filter(([group]) => group === activeGroup).map(
          ([group, description]) => (
            <SettingsGroup
              key={group}
              group={group}
      ...
      <div className="pp-settings-actions">
        ...
        {validationError && (
          <p className="pp-error-text" role="alert">
            {validationError}
          </p>
        )}
        {mutation.error && (
        ...
        {dirty && data.revision > saved.revision && (
          <p className="pp-error-text" role="alert">
            A colleague saved a newer version. Discard your draft and reload
            saved settings before editing again.
          </p>
        )}
      </div>
      <div className="pp-settings-tools">
        ...
        <button
          className="pp-btn"
          type="button"
          disabled={dirty || mutation.isPending || reloading}
          onClick={reload}
        >
          Reload saved settings
        </button>
        <p>
          Reset prepares a draft. Save changes to publish it. Visitors receive
          saved settings the next time they open or reload the app.
        </p>
      </div>
    </form>
  );
...
export default function AppSettingsPage() {
  ...
  return (
    <div className="pp-page">
      ...
      {query.error && (
        <div role="alert">
          <p className="pp-error-text">{query.error.message}</p>
          <button
            className="pp-btn pp-btn-inline"
