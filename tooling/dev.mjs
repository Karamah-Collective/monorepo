...
try {
  ...
  if(!['all','admin','maps','website'].includes(target))throw new Error('Use maps, website, or admin.');
  if(target!=='website' && !await busy(8788)) { console.log('Preparing the existing local Maps database…');await setupLocalDatabase();launch(wrangler,['pages','dev','.', '--port','8788','--ip','127.0.0.1'],path.join(root,'Maps')); }
  if(target!=='maps' && !await busy(8789)) launch(wrangler,['pages','dev','.', '--port','8789','--ip','127.0.0.1'],path.join(root,'Website'));
  ...
  console.log('Maps: http://127.0.0.1:8788 · Website: http://127.0.0.1:8789 · Admin: http://127.0.0.1:5173');
