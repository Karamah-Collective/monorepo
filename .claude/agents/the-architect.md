...
- Prefer **Edit** over **Write** for existing files — it only touches the diff and is safer for large files like `src/styles/styles.css` or `src/places.js`.
- Before deleting any CSS class or JS export, grep the whole `src/` (and `functions/`) tree for its exact name first — a class/function can look exclusive to the feature you're touching while actually being shared by something else (e.g. `.place-popup-wrap` looked place-detail-specific but was also load-bearing for the dropped-pin and current-location popups).
