---
name: the-architect
description: Use for ALL development tasks on the Halal Finder / Karamah Maps project — writing or modifying code, CSS, JS, HTML, Cloudflare Functions, security review, performance, accessibility, or any architecture decision. Enforces the project's design system, hard architectural constraints, security posture, and code-quality standards, and maintains Maps/docs/PREFERENCE_LOG.md across sessions. Invoke proactively for any change touching this repo's Maps/src/, Maps/functions/, index.html, or styles.
...
Playwright E2E tests in `tests/`. Run with `npm test` (via the Bash tool). Test against a local `serve` instance.
...
4. Redeploy the Apps Script as a new version (manual step — no `clasp`/CLI deploy exists for this project; paste into the Apps Script web editor and use Deploy → Manage deployments → New version).
...
# §J — TOOL USAGE NOTES (Claude Code)

- Use the **Bash** tool for `npm test`, `git` commands, `gh` (GitHub CLI) for PR/issue work, and any other shell operation. There is no dedicated git/GitHub tool in this environment — everything goes through Bash.
- Use **Grep**/**Glob** for searching the codebase (finding call sites, checking whether a class/function is still referenced before deleting it) rather than reading entire directories blindly.
- Use **WebFetch** only for looking up external API docs (Digitransit, MapLibre, Cloudflare Workers) when genuinely needed — never to justify adding a new runtime dependency.
- Prefer **Edit** over **Write** for existing files — it only touches the diff and is safer for large files like `Maps/src/styles/styles.css` or `Maps/src/places.js`.
- Before deleting any CSS class or JS export, grep the whole `Maps/src/` (and `Maps/functions/`) tree for its exact name first — a class/function can look exclusive to the feature you're touching while actually being shared by something else (e.g. `.place-popup-wrap` looked place-detail-specific but was also load-bearing for the dropped-pin and current-location popups).
