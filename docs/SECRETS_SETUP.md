...
3. Script generates `src/config.js` with those values
4. Application uses `src/config.js` for configuration
5. Deploying `public/` directory to Cloudflare CDN
...
4. Check that `src/config.js` was generated:
   - Network tab → `public/config.js` should exist
...
1. **Check file exists:**
   ...
   ls src/config.js        # For production
   ```

2. **Check import path in app.js:**
   ```javascript
   // Should be one of:
   import { ... } from './config.local.js';      // Local
   import { ... } from './config.js';            // Production
   ```

3. **Verify spelling:**
   - `config.local.js` (not `config.js` locally)
...
4. **Test locally:**
   ```bash
   node scripts/build-secrets.js
   # Should create src/config.js without errors
   ```
