...
- [x] **`halal-finder-admin` Pages project build fixed and live** — first build failed (`"build output directory is outside of the repository"`, caused by Cloudflare auto-detecting the repo-root `wrangler.toml`'s `pages_build_output_dir = "."` instead of the dashboard's `dist` setting); fixed by adding `admin/wrangler.toml` (`pages_build_output_dir = "dist"`), pushed (`5c4a878`), build succeeded.
- [x] Deployed admin app confirmed working through signup → email verification → login → signed-in Dashboard view. Data fetch failed with "Failed to fetch" — expected, since production's `/api/admin` was still on old adminKey code (CORS preflight only allowed `Content-Type`, not `Authorization`, so the browser blocked the request before it even reached the backend).
- [x] **Ran the full "deploy" promotion** (`.github/prompts/deploy.prompt.md`, "deploy" command): `npm run update:force` (VERSION 20260805 → 20260805-2, places unchanged at 185), committed + pushed to `main` (`3af7f85`), promoted `functions/`, `src/`, etc. directly to `deploy` (`37929e0`), skipping `preview`. Production now has the new Firebase-authed `functions/api/admin.js`.
- [ ] **Next**: confirm the admin dashboard's "Failed to fetch" is now resolved — reload the dashboard and check that real data loads.
